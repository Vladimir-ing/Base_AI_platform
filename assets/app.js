...
async function callRemoteAssistant(query) {
  const client = window.supabaseClient;
  if (!client) throw new Error("auth_unavailable");

  const { data, error } = await client.functions.invoke("ai-base-assistant", {
    body:{query:String(query||"").slice(0,1200),platforms:assistantSafePlatforms()}
  });
  if (error) {
    let code = error?.message || "llm_unavailable";
...
async function flowLocalAssistant() {
  const body = "<div class='assistant-ui'>" +
    ...
    "<div class='assistant-connect'><div><b>LLM через защищённый аккаунт</b><small>Авторизация выполняется автоматически через Supabase</small></div></div>" +
  ...
  await modal({
    ...
    onOpen: root => {
      ...
      const run = async q => {
        ...
        try {
          const data = await callRemoteAssistant(q);
...
function showHelp() {
  modal({
    sub: "Короткий маршрут от входа до резервной копии",
    body: `
      <p class="guide-intro">База помогает хранить AI-сервисы, доступы, тарифы, даты списаний и рабочие приёмы в одном месте.</p>
      <div class="guide-steps">
        <div class="guide-step"><span>1</span><div><b>Найдите или добавьте платформу</b><p>Используйте поиск и фильтры. Для нового сервиса нажмите «+ Платформа».</p></div></div>
        <div class="guide-step"><span>2</span><div><b>Заполните карточку</b><p>Откройте плитку → «Редактировать»: добавьте ссылки, статус, тариф, дату списания, оценку и заметки.</p></div></div>
        <div class="guide-step"><span>3</span><div><b>Создайте сейф для секретов</b><p>Нажмите «Создать сейф» и задайте мастер-пароль. Только после этого сохраняйте пароли, API-ключи и приватные заметки.</p></div></div>
        <div class="guide-step"><span>4</span><div><b>Настройте оплату</b><p>В поле «Чем платим» укажите карту, счёт или сервис-посредник. Сводка покажет расходы и ближайшие списания.</p></div></div>
        <div class="guide-step"><span>5</span><div><b>Используйте анализ</b><p>«Сравнить» сопоставляет несколько платформ, а «Помощник» отвечает по данным вашей базы без передачи логинов и секретов.</p></div></div>
        <div class="guide-step"><span>6</span><div><b>Проверьте сохранение</b><p>Дождитесь статуса «Сохранено». Периодически делайте «⋯ → Экспорт бэкапа» как независимую копию.</p></div></div>
      </div>
      <div class="sect"><h4>Статус облака</h4><dl class="guide-status">
        <div><dt>Сохранено</dt><dd>Изменения записаны в Supabase.</dd></div>
        <div><dt>Синхронизация…</dt><dd>Идёт отправка или загрузка данных.</dd></div>
...
$("#assistantBtn").addEventListener("click", flowLocalAssistant);
...
{ name:"OpenRouter", url:"https://openrouter.ai", category:"Текст/ассистенты",
  purpose:"Единый API-шлюз к сотням моделей разных вендоров: один ключ и один счёт вместо кучи аккаунтов.",
  strengths:"Один баланс и один ключ для OpenAI, Anthropic, Google, Meta и других; прозрачная оплата за токены, есть бесплатные модели; удобное сравнение моделей по цене и качеству.",
  tips:["Пополняйте баланс и платите по факту за токены — подписка на каждую модель не нужна.",
  ...
  tags:["api","агрегатор","текст"] },

{ name:"MiMo", url:"https://mimo.mi.com", category:"Текст/ассистенты",
  purpose:"Агентная платформа Xiaomi на моделях MiMo: чат, код и пошаговые задачи с вызовом инструментов.",
  strengths:"Быстрые открытые модели линейки MiMo-V2, агентный режим, API и подписка на токены; сильное соотношение цены и качества.",
  tips:["Один аккаунт Xiaomi открывает и чат, и API — ключ держите в сейфе этой базы.",
  ...
  tags:["текст","агент","api"] },

{ name:"GLM", url:"https://z.ai", category:"Текст/ассистенты",
  purpose:"Чат и API на моделях GLM от Z.ai: сильный код, агентные задачи и рассуждения.",
  strengths:"Открытые веса моделей, низкие цены; Coding Plan подключается к Claude Code, Cline и другим инструментам вместо оплаты по токенам.",
  tips:["Если много кодите — берите Coding Plan (от $18/мес): квоты вместо токенов, лимиты на 5 часов и на неделю.",
